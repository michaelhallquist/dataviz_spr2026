...
project_directory <- "~/TresorSync/Teaching/Data Management and Visualization UNC/dataviz_spr2026/materials/w03_automation"
...
ibifiles <- list.files(path=file.path(project_directory, "ecg_physio"), pattern="negint", full.names=TRUE)
...
ids <- substr(basename(ibifiles), 1, 4)

# position <- sub(".*/8[0-9]{3}_([lr])_.*", "\\1", ibifiles, perl=TRUE, ignore.case = TRUE)
position <- substr(basename(ibifiles), 6, 6)
...
read_couple <- function(filename) {
  id <- substr(basename(filename), 1, 4)
  position <- substr(basename(filename), 6, 6)
  dataset <- read.table(filename, col.names=c("time", "ibi"))
  dataset$PTNUM <- factor(id)
  dataset$position <- factor(position)
  
  return(dataset)
}

test <- read_couple(ibifiles[2])

allibi <- lapply(ibifiles, read_couple)

output <- lapply(1:15, function(x) print(x - 5))
