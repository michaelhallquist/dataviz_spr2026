...
date: today
week: 1
download: "materials/downloads/w01_tidy_management/tidy_data_conceptual.qmd"
format:
  html:
    code-fold: show
---
...
mess1 <- readr::read_csv("../files/religion_messy.csv")
```
...
```{r, echo=FALSE, message=FALSE}
mess2 <- readr::read_csv("../files/weather_messy.csv")
```
...
```{r, echo=FALSE, message=FALSE}
mess3 <- readr::read_csv("../files/billboard_messy.csv")
```
...
```{r, echo=FALSE, message=FALSE}
readr::read_csv("../files/tidy3.csv") %>% kable_table(n=8) 
```
